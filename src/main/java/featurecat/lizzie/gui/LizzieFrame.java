...
public class LizzieFrame extends JFrame {
  ...
  public LizzieFrame() {
        ...
        mainBackgroundPanel =
            ...
            protected void paintComponent(Graphics g) {
              if (Config.isScaled) {
            	...
            	paintBackgroundPane(g);
            }
          };
          mainBackgroundPanel.enableInputMethods(false);
    ...
    basePanel.add(mainBackgroundPanel, new Integer(10));
  ...
  private BufferedImage cachedBackImage;
...
//    }
  ...
  private void drawCommentBackground(Graphics2D g, int x, int y, int w, int h) {
	    if (isCommentArea) {
	      g.setColor(Lizzie.config.commentBackgroundColor);
	      g.fillRect(x, y, w, h);
	    }
  ...
  public void paintStonePane(Graphics g0)
  {
	  
  }
  
  public void paintBackgroundPane(Graphics g0)
  {	
	      isSmallCap = false;
	      int width = mainBackgroundPanel.getWidth();
	      int height = mainBackgroundPanel.getHeight();
	      ...
	      if (!showControls) {
	        ...
	        g.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
	        if (Lizzie.config.isFourSubMode()) {
	          int topInset = mainBackgroundPanel.getInsets().top;
	          int leftInset = mainBackgroundPanel.getInsets().left;
	          int rightInset = mainBackgroundPanel.getInsets().right;
	          int bottomInset = mainBackgroundPanel.getInsets().bottom;
	          ...
	          boolean noCommentAndListPane = noComment && noListPane;
	          // board
	        int  subMaxSize = (int) (min(width - leftInset - rightInset, height - topInset - bottomInset));
	          subMaxSize = max(subMaxSize, max(Board.boardWidth, Board.boardHeight) + 5);
	          subBoardRenderer.setLocation(topInset, leftInset);
	          subBoardRenderer.setBoardLength(subMaxSize / 2, subMaxSize / 2);
	          subBoardRenderer.drawBackground(g);

	          subBoardRenderer2.setLocation(subMaxSize / 2, leftInset);
	          subBoardRenderer2.setBoardLength(subMaxSize / 2, subMaxSize / 2);
	          subBoardRenderer2.drawBackground(g);

	          subBoardRenderer3.setLocation(topInset, subMaxSize / 2);
	          subBoardRenderer3.setBoardLength(subMaxSize / 2, subMaxSize / 2);
	          subBoardRenderer3.setupSizeParameters();
	          subBoardRenderer3.drawBackground(g);

	          subBoardRenderer4.setLocation(subMaxSize / 2, subMaxSize / 2);
	          subBoardRenderer4.setBoardLength(subMaxSize / 2, subMaxSize / 2);
	          subBoardRenderer4.drawBackground(g);
	          ...
	          boolean isWidth = trueWidth * 0.72 > trueHeight;
	          if (isWidth) {
	        	  int  maxSize = (int) (min(trueWidth, trueHeight));
	            maxSize = max(maxSize, max(Board.boardWidth, Board.boardHeight) + 5);
	            int  boardX = width - maxSize; // ) / 8 * BoardPositionProportion;
	            int   boardY = trueHeight - maxSize;
	            boardRenderer.setLocation(boardX, boardY);
	            boardRenderer.setBoardLength(maxSize, maxSize);
	            boardRenderer.drawBackground(g);
	            ...
	            if (!noVariation) {
	              if (!noCommentAndListPane) {
	                if (noWinrate && noBasic) {
	                	 {if (backgroundG.isPresent()) 
	                	  drawContainer(backgroundG.get(),vx, vy, vw, vh);
	                	  if (noListPane) drawCommentBackground(g, vx, vy + vh / 2, vw, vh / 2);
	                	 }
	                } else {
	                  if (backgroundG.isPresent()) 
	                	  drawContainer(backgroundG.get(),vx,  vy + vh / 2, vw, vh/2);
	                	  if (noListPane) drawCommentBackground(g, vx, vy + vh * 3 / 4, vw, vh / 4);
	                }
	              } else {
	                if (noWinrate && noBasic) {
	                	if (backgroundG.isPresent()) 
	            ...
	            if (!noWinrate) {
	              ...
	              if (!noBasic) {	            	  
	                drawWinrateBackground(g,vx, vy + vh / 4, vw,  vh / 4);
	              } else {
	                drawWinrateBackground(g,vx, vy, vw,  vh / 2);
	              }
	            } else if (!noBasic) {
	            	 if (backgroundG.isPresent()) 
	                	  drawContainer(backgroundG.get(),subMaxSize,  0, vw, vh/2);
	            ...
	            int maxSize = (int) (min(trueWidth, 0.77 * trueHeight));
	            maxSize = max(maxSize, max(Board.boardWidth, Board.boardHeight) + 5);
	            int boardX = subMaxSize; // ) / 8 * BoardPositionProportion;
	            int boardY = trueHeight - maxSize;
	            boardRenderer.setLocation(boardX, boardY);
	            boardRenderer.setBoardLength(maxSize, maxSize);
	            boardRenderer.drawBackground(g);
	            ...
	            if (!noVariation) {
	              if (noWinrate && noBasic) {
	            	   if (backgroundG.isPresent()) 
	                 	  drawContainer(backgroundG.get(),vx,  vy, vw, vh);
	                if (!noCommentAndListPane) {
	                	if (noListPane) drawCommentBackground(g, vx, vy, vw / 2, vh);
	                } 
	              } else {
	            	  if (backgroundG.isPresent()) 
	                 	  drawContainer(backgroundG.get(),vx,  vy+vh/2, vw, vh/2);
	                if (!noCommentAndListPane) { 
	                	if (noListPane) drawCommentBackground(g, vx, vy + vh / 2, vw / 2, vh / 2);	                 
	                }
	              }
	            } else if (noWinrate && noBasic) {
	            	if (backgroundG.isPresent()) 
	                 	  drawContainer(backgroundG.get(),vx,  vy, vw, vh);
	               if (noListPane) drawCommentBackground(g, vx, vy, vw, vh);
	            } else {
	              if (!noCommentAndListPane) {
	            	  if (backgroundG.isPresent()) 
	                 	  drawContainer(backgroundG.get(),vx,  vy+vh/2, vw, vh/2);
	            ...
	            if (!noWinrate) {
	              if (noCommentAndListPane && noVariation) {
	            	   if (backgroundG.isPresent()) 
		                 	  drawContainer(backgroundG.get(),vx,  vy, vw, vh);
	                if (!noBasic) {
	                  drawWinrateBackground(g,vx + vw / 2, vy,  vw / 2, vh);	                  
	                } else {
	                  drawWinrateBackground(g,vx, vy,  vw , vh);
	                }
	              } else {
	            	  if (backgroundG.isPresent()) 
	                 	  drawContainer(backgroundG.get(),vx,  vy, vw, vh/2);
	                if (!noBasic) {
	                  drawWinrateBackground(g,vx + vw / 2, vy, vw / 2, vh / 2);	                
	                } else {
	                  drawWinrateBackground(g,vx, vy, vw, vh / 2);		                
	                }
	              }
	        ...
	        // cleanup
	        g.dispose();
	        this.cachedBackImage = cachedImage;
	      }
	    
	    // draw the image
	    g0.drawImage(cachedBackground, 0, 0, null);
	    g0.drawImage(cachedBackImage, 0, 0, null);
  }
  
  public void paintInfoPane(Graphics g0) {
	    if (this.redrawWinratePaneOnly) {
	    	drawWinratePaneInfo(this.grx, this.gry, this.grw, this.grh);
	      redrawWinratePaneOnly = false;
	    } else 
	    {
	      isSmallCap = false;
...
//	      }
	          ...
	          if (isWidth) {
	            ...
	            if (!noWinrate) {
	              if (!noBasic) {
	                ...
	                gry = vy + vh / 4;
	                grh = vh / 4;
	                drawWinratePaneInfo(grx, gry, grw, grh);
	                // winrateGraph.draw(g, grx, gry, grw, grh);
	                statx = vx;
	                staty = vy;
	                statw = vw;
  ...
  private void drawWinrateBackground(Graphics2D g,int x, int y, int w, int h) {
   	   int halfHeight = h / 2;
   ...
   g.setPaint(gradient);
   g.fillRect(x, y, w, halfHeight);
   g.setPaint(gradient2);
   g.fillRect(x, y + halfHeight, w, h - halfHeight);
   g.setPaint(original);
  }
  
  
  private void drawWinratePaneInfo(int x, int y, int w, int h) {	   
	    	 if (w < 10 || h < 10) {
	   	      cachedWinrateImage = new BufferedImage(1, 1, TYPE_INT_ARGB);
	   	      return;
	   	    }
